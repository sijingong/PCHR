...
public final class RandomCodeUtils {
	...
	private final static char MCODE[] = { '1', '2', '3', '4', '5', '6', '7', '8', '9', '0' };
	...
	public final static String bulidNoRepeatCodes(int codeLength) {
		if (codeLength > CODES_COUNT || codeLength < 1) {
			throw new IllegalArgumentException("Random code length can not less than 1 or greater than " + CODES_COUNT);
		}
	...
	public final static String bulidNoRepeatCodes(char codes[], int codeLength) {
		...
		if (codeLength > cout || codeLength < 1) {
			throw new IllegalArgumentException("Random code length can not less than 1 or greater than" + cout);
		}
		StringBuffer codesBuf = new StringBuffer();
		char code;
		Random random = new Random();
	...
	public final static String bulidNoRepeatReverseCodes(char codes[], int codeLength) {
		if (codeLength < 1) {
			throw new IllegalArgumentException("Random code length can not less than 1");
		}
		char difCodes[] = getDifCodes(codes);
		int length = difCodes.length;
		if (length < codeLength) {
			throw new IllegalArgumentException("The differ random code libraries length can not less than " + codeLength);
		}
		StringBuffer codesBuf = new StringBuffer();
		char code;
		Random random = new Random();
		...
		while (codesBuf.length() < codeLength) {
			...
			if (-1 == codesBuf.toString().indexOf(code) && -1 == codesStr.indexOf(code)) {
				codesBuf.append(code);
			} else {
				continue;
			}
	...
	public final static String bulidRepeatReverseCodes(char codes[], int codeLength) {
		if (codeLength < 1) {
			throw new IllegalArgumentException("Random code length can not less than 1");
		}
		char difCodes[] = getDifCodes(codes);
